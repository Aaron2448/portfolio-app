...
//		return new InMemoryUserDetailsManager(admin1, user2);
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http.csrf().disable()
                ...
                .requestMatchers("/api/v1/user/authenticate", "/api/v1/user/register", "/api/v1/prediction/checkIp").permitAll()
                .and()
                .authorizeHttpRequests().requestMatchers("/api/v1/user/getProfile", "/api/v1/user/logout").authenticated()
                .and()
                .sessionManagement()
